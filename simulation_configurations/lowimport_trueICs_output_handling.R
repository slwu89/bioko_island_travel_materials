"qsub -l m_mem_free=8G -l fthread=1 -P proj_mmc -q all.q -N lowimport_output_handling -o /ihme/malaria_modeling/dtcitron/SpatialUncertainty/ASTMH19/lowimport_trueICs/log_files -e /ihme/malaria_modeling/dtcitron/SpatialUncertainty/ASTMH19/lowimport_trueICs/log_files /ihme/singularity-images/rstudio/shells/execRscript.sh -s /ihme/malaria_modeling/dtcitron/SpatialUncertainty/ASTMH19/lowimport_trueICs_output_handling.R"
...
ensemble.file.list <- list.files(path = here("SpatialUncertainty/ASTMH19/lowimport_trueICs/sim_output"),
                                 pattern = "pfsi_[[:digit:]]+.csv")


# load in the first one
df_curr <- fread(here("SpatialUncertainty/ASTMH19/lowimport_trueICs/sim_output",ensemble.file.list[1]))
...
for (i in 2:nrun){
  df_curr <- fread(here("SpatialUncertainty/ASTMH19/lowimport_trueICs/sim_output",ensemble.file.list[i]))
...
fwrite(mat_mean, here("SpatialUncertainty/ASTMH19/lowimport_trueICs/lowimport_trueICs_pr_means_1000.csv"))
fwrite(mat_sd, here("SpatialUncertainty/ASTMH19/lowimport_trueICs/lowimport_trueICs_pr_sds_1000.csv"))
