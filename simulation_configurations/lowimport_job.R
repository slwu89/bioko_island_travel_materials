...
pfpr <-  (TaR.matrix %*% c(h.FOI[1:241], b*eg.eir))/((1 + r*rho/eta/(1-rho))*(TaR.matrix %*% c(h.FOI[1:241], b*eg.eir)) + r/(1-rho)) #pfpr.input # Lower Importations - nonzero but smaller h off-island
...
h_inf <- here("SpatialUncertainty/ASTMH19/lowimport/sim_output", paste0("pfsi_", seed, ".csv"))
...
mosy <-  here("SpatialUncertainty/ASTMH19/lowimport/sim_output", paste0("mosy_", seed, ".csv"))
...
# h_inf <- here("SpatialUncertainty/ASTMH19/lowimport/sim_output", paste0("pfsi_", 2, ".csv"))
# dt <- fread(h_inf)
# 
