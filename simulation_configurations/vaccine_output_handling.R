...
ensemble.file.list <- list.files(path = here("SpatialUncertainty/ASTMH19/vaccine/sim_output"),
                                 pattern = "pfsi_[[:digit:]]+.csv")

# load in the first one
df_curr <- fread(here("SpatialUncertainty/ASTMH19/vaccine/sim_output",ensemble.file.list[1]))
...
for (i in 2:nrun){
  df_curr <- fread(here("SpatialUncertainty/ASTMH19/vaccine/sim_output",ensemble.file.list[i]))
...
fwrite(mat_mean, here("SpatialUncertainty/ASTMH19/vaccine/vaccine_pr_means_1000.csv"))
fwrite(mat_sd, here("SpatialUncertainty/ASTMH19/vaccine/vaccine_pr_sds_1000.csv"))