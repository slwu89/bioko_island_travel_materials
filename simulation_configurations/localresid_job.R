##
#
# Script for Outputting Local Residual Bioko Island Simulations
# For ASTMH
...
# This is for Local Residual Fraction, what happens when we turn off exposure to Malaria risk on the mainland
#
# To clarify: zero malaria risk on the mainland
#
...
pfpr.input <- c(pop.data$pfpr, 0.43) # Local Residual: This is the PfPR input; it is not the same as kappa, which we do use to parameterize lambda etc
...
kappa[242] <- 0 # Local Residual: here is where we set the off-island kappa to 0
...
eg.eir <- 0 # Local residual faction: no off-island transmission #h.FOI[242]/b
...
pfpr <-  (TaR.matrix %*% c(h.FOI[1:241], 0))/((1 + r*rho/eta/(1-rho))*(TaR.matrix %*% c(h.FOI[1:241], 0)) + r/(1-rho)) #pfpr.input # Local Residual - need to anticipate a small change in LR after changing off-island travel
...
h_inf <- here("SpatialUncertainty/ASTMH19/localresid/sim_output", paste0("pfsi_", seed, ".csv"))
...
mosy <-  here("SpatialUncertainty/ASTMH19/localresid/sim_output", paste0("mosy_", seed, ".csv"))
...
# h_inf <- here("SpatialUncertainty/ASTMH19/localresid/sim_output", paste0("pfsi_", 2, ".csv"))
# dt <- fread(h_inf)
# 
