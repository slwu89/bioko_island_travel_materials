##
#
# Script for Outputting Travel Fraction Bioko Island Simulations
# For ASTMH
...
# Travel fraction:
# We will turn off local EIR in all locations except for off-island
# We will see how off-island infections alone drive the transmission everywhere
...
set.seed(seed)
#pfpr.draw.ix <- sample(1:100, 1) # TF: don't need this
...
# PfPR data
#pfpr.data <- fread(here("SpatialUncertainty/data_clean/pfpr_draws.csv")) # TF: don't need this
#pfpr.data <- merge(pfpr.data, pop.data[year == 2018, .(areaId)], by = "areaId", all = FALSE) # TF: don't need this
...
# Derive EIR ####
#pfpr.draw.colname = paste0("draw.", pfpr.draw.ix) # TF: don't need this
#pfpr.data.draw = pfpr.data[,c("areaId", pfpr.draw.colname), with = FALSE] # TF: don't need this
#colnames(pfpr.data.draw)[2] = "pfpr" # TF: don't need this
#pop.data <- merge(pop.data[year == 2018], pfpr.data.draw, by = "areaId") # TF: don't need this
...
M = rep(0, 242) # h.FOI*H.visitors/a/b/z.spz # TF: don't need this

Z = rep(0, 242) # z.spz * M # TF: don't need this
#Z[242] = 0 # for off-island # TF: don't need this
Y = rep(0, 242) # Z/peip # TF: don't need this
...
Lambda = rep(0,242) # M*(1-p)/p # TF: don't need this
#Lambda[242] = 0 # for off-island # TF: don't need this
...
n.patch <- 242 # 194 + 1 : the last patch is off-island
...
h_inf <- here("SpatialUncertainty/ASTMH19/travelfrac/sim_output", paste0("pfsi_", seed, ".csv"))
...
mosy <-  here("SpatialUncertainty/ASTMH19/travelfrac/sim_output", paste0("mosy_", seed, ".csv"))
...
# h_inf <- here("SpatialUncertainty/ASTMH19/travelfrac/sim_output", paste0("pfsi_", 3, ".csv"))
# dt <- fread(h_inf)
# 
