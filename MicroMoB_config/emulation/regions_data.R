...
TaR_cluster <- TaR_cluster / rowSums(TaR_cluster)

write.csv2(TaR_cluster, here("MicroMoB_config/emulation/tar.csv"))

pfpr_cluster <- unlist(lapply(X = clusters_ix, FUN = function(x) {mean(pfpr.data$draw.mean[x])}))
pfpr_cluster_lo <- unlist(lapply(X = clusters_ix, FUN = function(x) {quantile(pfpr.data$draw.mean[x], probs = 0.025)}))
pfpr_cluster_hi <- unlist(lapply(X = clusters_ix, FUN = function(x) {quantile(pfpr.data$draw.mean[x], probs = 0.975)}))

N_cluster <- unlist(lapply(X = clusters_ix, FUN = function(x) {
  pop.data[year == 2018, sum(pop[x])]
}))
...
odds <- r/(1-rho)*pfpr_cluster/(1-(1+rho*r/eta/(1-rho))*pfpr_cluster)
