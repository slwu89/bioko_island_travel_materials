...
# S <- N - I - ((rho*I*r)/(eta*(1-rho)))
# P <- N - I - S
# h <- (eta * P) / (rho * S) # the FOI experienced by each strata (summing over their movements)
# 
...
# H <- as.vector(H)
# 
# # density of infectious mosy calc from ambient pop
# Z <- (EIR * H) / a
# 
# # kappa
...
# odds <- r/(1-rho)*pfpr/(1-(1+rho*r/eta/(1-rho))*pfpr)
# 
# # Force of Infection (FOI) on each strata
# h_strata <- as.vector(MASS::ginv(TaR) %*% odds)
# h_strata <- pmax(h_strata, 0)
# 
# 
# # ----------
# # calc without using Rio Muni PfPR at all
# 
# pfpr <- pfpr.data$draw.mean
# pfpr[c(71,72,109,215,216)] <- pfpr[c(71,72,109,215,216)] * 1.5
# N <- pop.data[year == 2018]$pop
...
# h <- (eta * P) / (rho * S) # the FOI experienced by each strata (summing over their movements)
# 
# EIR_norio <- MASS::ginv(TaR) %*% h/b # the EIR produced by mosquitoes at each patch
# EIR_norio <- as.vector(EIR_norio)
