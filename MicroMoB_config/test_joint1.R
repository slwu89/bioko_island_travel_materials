...
while (get_tnow(mod) <= tmax) {
  ...
  mod$human$EIR <- mod$human$EIR + (TaR[, 242] * reservoir_eir)
...
parout <- parallel::mclapply(X = 1:50, FUN = function(runid) {
  ...
  # sample the mosquito state stochastically to get the right total pop
  ...
  setup_mosquito_RM(model = mod, stochastic = TRUE, f = f, q = q, eip = eip, p = p, psi = diag(n_patch), nu = 25, M = MYZ_sto[, 'M'], Y = MYZ_sto[, 'Y'], Z = MYZ_sto[, 'Z'])
